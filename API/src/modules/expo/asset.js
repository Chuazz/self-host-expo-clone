...
module.exports.handleAssetData = ({ query: { asset, contentType } }) => {
  ...
  if (
    asset.includes("app.json") ||
    asset.includes("package.json") ||
    !asset.startsWith("/updates/")
  ) {
...
module.exports.getJSONInfo = ({ path: paramPath }) => {
  ...
  if (!fs.existsSync(appJsonPath))
    throw new Err.GeneralError("Error: app.json not found");
  if (!fs.existsSync(pkgJsonPath))
    throw new Err.GeneralError("Error: package.json not found");
