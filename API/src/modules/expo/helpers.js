...
module.exports.getAssetMetadataSync = ({
  update,
  filePath,
  ext,
  isLaunchAsset,
  ...
  return {
    ...
    }/api/assets?asset=${assetFilePath}&contentType=${encodeURI(
      contentType
    )}&platform=${platform}`,
  };
};
...
module.exports.getMetadataSync = (update) => {
  try {
    ...
    throw new Error(
      `No update found with runtime version: ${update.version}. Error: ${error}`
    );
  }
};
