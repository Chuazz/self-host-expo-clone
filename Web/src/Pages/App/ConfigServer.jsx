...
export const ConfigServer = ({ state: [update, setUpdate] }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await FC.client
        .service("apps")
        .patch(_id, { ...fields, lastUpdate: new Date() });
      invalidateQuery("app");
      window.toast.show({
        severity: "info",
  ...
  return (
    <Card
      title={`SERVER CONFIGURATION ${
        needsSave ? " (MODIFIED, YOU NEED TO SAVE)" : ""
      }`}
      collapsable
      collapsed
      fadeIn
    ...
    >
      <Flex as style={{ padding: 10 }}>
        ...
        <Text
          value="App Certificate (download and add this certificate to your app.json):"
          style={{ marginTop: 20 }}
        />
        <Input
          multiline
        ...
        <Text
          value="Private Key (used only on the server):"
          style={{ marginTop: 40 }}
        />
        <Input
          multiline
        ...
        {loading ? (
          ...
          <Button label="Save" onClick={handleSave} style={{ marginTop: 40 }} />
        )}
      </Flex>
    </Card>
  );
